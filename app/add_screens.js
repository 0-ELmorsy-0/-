    ...
    Scaffold(
        ...
        Column(modifier = Modifier.fillMaxSize().padding(padding).padding(16.dp).verticalScroll(rememberScrollState())) {
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                VitalCard("الضغط", recentBloodPressure.ifBlank { "--" }, Icons.Default.FavoriteBorder, Modifier.weight(1f))
                Spacer(Modifier.width(8.dp))
                VitalCard("النبض", recentHeartRate.ifBlank { "--" }, Icons.Default.Favorite, Modifier.weight(1f))
                Spacer(Modifier.width(8.dp))
                VitalCard("السكر", recentBloodSugar.ifBlank { "--" }, Icons.Default.WaterDrop, Modifier.weight(1f))
            }
            ...
            Button(onClick = { 
                ...
                viewModel.updateMedicalProfile(
                    patientAge, patientBloodType, patientWeight, patientGender, patientConditions, patientAllergies, emergencyContactName, emergencyContactPhone,
                    if(newVitalType == "HEART") newValue else recentHeartRate,
                    if(newVitalType == "PRESSURE") newValue else recentBloodPressure,
                    if(newVitalType == "SUGAR") newValue else recentBloodSugar
                )
                newValue = ""
            }, modifier = Modifier.fillMaxWidth()) {
                Text("تسجيل القياس")
